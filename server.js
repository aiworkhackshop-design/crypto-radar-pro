...
async function fetchJson(url) { const response = await fetch(url, { headers: { accept: 'application/json', 'user-agent': 'crypto-war-room/3.0' } }); if (!response.ok) throw new Error(`${response.status} ${url}`); return response.json(); }
...
function computeBacktest(candles) {
  ...
  for (let i = 10; i < candles.length - 6; i++) {
    ...
    const move = pct(recent[recent.length - 1].close, recent[0].open);
...
function createSignals(market) {
  ...
  if (volume[0]) signals.push({ type: 'VOLUME_RADAR', title: '出来高レーダー', tone: volume[0].volumeSpike >= 1.25 ? 'go' : 'wait', body: `${volume[0].name}: 出来高 ${volume[0].volumeSpike.toFixed(2)}x / 6h ${volume[0].trend6h.toFixed(2)}%` });
...
async function fetchMarket() {
  ...
  try {
    ...
    return { market: result.market, source: 'coinbase', warning: result.errors.length ? `一部銘柄取得失敗: ${result.errors.length}件` : null };
  } catch (error) {
    if (state.lastMarket.length) return { market: state.lastMarket, source: 'cache', warning: `${error.message} / キャッシュ表示中` };
