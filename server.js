...
function analyze(m) {
  ...
  if (score>=72 && warnings.length<=1) { decision='BUY_WATCH'; action='候補。直近高値更新か押し目反発を確認。'; }
...
function signals(market, news) {
  ...
  const out=[];
  if (buy[0]) out.push({ type:'ENTRY_CANDIDATE', title:'候補あり', tone:'go', body:`${buy[0].name}: score ${buy[0].score} / ${buy[0].action}` });
...
async function emitAlerts(sig) {
  ...
  for (const s of sig.filter(x=>['ENTRY_CANDIDATE','NEWS_ALERT'].includes(x.type) || (x.type==='VOLUME_RADAR' && x.tone==='go'))) {
    ...
    if(notificationEnabled){ const sent=await pushLine(`Crypto Signal Cockpit\n${s.title}\n${s.body}`); item.notified=sent.ok; item.notifyError=sent.reason||null; }
    created.push(item);
  }
...
const server=http.createServer(async(req,res)=>{const url=new URL(req.url,`http://${req.headers.host}`);if(req.method==='OPTIONS')return json(res,200,{ok:true});
 ...
 if(req.method==='GET'&&url.pathname==='/api/market'){
  ...
  state.lastUpdated=new Date().toISOString();const sig=signals(result.market,news);const newAlerts=await emitAlerts(sig);const actionables=result.market.filter(c=>c.decision!=='NO_TRADE').slice(0,5);
  return json(res,200,{updatedAt:state.lastUpdated,source:'coinbase',warning:result.errors.length?`一部取得失敗: ${result.errors.length}件`:null,signals:sig,alerts:state.alerts.slice(0,20),newAlerts,news,notification:{enabled:notificationEnabled,lineConfigured:notificationEnabled},top:actionables,market:result.market,portfolio:portfolioSummary(),trading:{paperTradingEnabled:false,pausedReason:'自動売買なし・監視のみ',consecutiveLosses:0},trades:[]});
 }
 ...
 if(req.method==='POST'&&url.pathname==='/api/notify/test')return json(res,200,await pushLine('Crypto Signal Cockpit test'));
 if(req.method==='GET')return serveStatic(req,res);return json(res,405,{error:'method_not_allowed'});
});
server.listen(PORT,'0.0.0.0',()=>console.log(`Crypto Signal Cockpit monitor-only running on port ${PORT}`));
